...
public class ezcontrol {
    ...
    public static String make_url(String ez_hostname, String system, String house, int device, String command_name)
        ...
        ezcontrol ez = new ezcontrol(ez_hostname);
    ...
    private static int system_no(String system_name) {
        if (system_name.equals("intertechno")) {
            return 6;
        } else if (system_name.equals("conrad")) {
            return 3;
        } else if (system_name.equals("x10")) {
            return 9;
        }
    ...
    private static String cmd2string(int cmd) {
        return cmd == commandnames.cmd_power_on
                ? "on" : cmd == commandnames.cmd_power_off ? "off" : "?";
    }

    public ezcontrol(String hostname, boolean verbose) {
        ezcontrol_host = hostname != null ? hostname : default_ezcontrol_host;
        this.verbose = verbose;
    }

    public ezcontrol(String hostname) {
        this(hostname, false);
    }

    public ezcontrol(boolean verbose) {
    ...
    public boolean send_preset(int switch_no, String cmd_name) throws non_existing_command_exception {
        return get_url(make_url_preset(switch_no, cmd_name));
    }
    ...
    public String intertechno_url(char house, int device, String cmd_name, int n)
        ...
        if (cmd == commandnames.cmd_invalid) {
            if (cmd_name.equals("on")) {
                cmd = commandnames.cmd_power_on;
            }
            if (cmd_name.equals("off")) {
                cmd = commandnames.cmd_power_off;
            }
        }

        if (cmd != commandnames.cmd_power_on && cmd != commandnames.cmd_power_off) {
    ...
    public String intertechno_url(int hc1, int hc2, int addr, int cmd, int n)
            throws non_existing_command_exception {
        return "http://" + ezcontrol_host + "/send?system=6&hc1=" + hc1 + "&hc2=" + hc2 + "&addr=" + addr + "&value=" + (cmd == commandnames.cmd_power_on ? "255" : "0") + "&n=" + n;
    }
    ...
    public String x10_url(int hc1, int addr, int cmd, int n)
            throws non_existing_command_exception {
        return "http://" + ezcontrol_host + "/send?system=9&hc1=" + hc1 + "&addr=" + addr + "&value=" + (cmd == commandnames.cmd_power_on ? "255" : "0") + "&n=" + n;
    }
    ...
    public String rs200_url(String house, int device, String cmd_name, int n)
        ...
        return "http://" + ezcontrol_host + "/send?system=3&hc1=" + house + "&addr=" + device + "&value=" + (cmd == commandnames.cmd_power_on ? "255" : "0") + "&n=" + n;
    }

    public String make_url_preset(int switch_no, String cmd_name)
            throws non_existing_command_exception {
        int cmd = ir_code.decode_command(cmd_name);
        if (cmd != commandnames.cmd_power_on && cmd != commandnames.cmd_power_off) {
            throw new non_existing_command_exception(cmd_name);
        }
        return "http://" + ezcontrol_host + "/preset?switch=" + switch_no + "&value=" + (cmd == commandnames.cmd_power_on ? "on" : "off");
    }
    ...
    public String get_preset_status() {
        setup_status();
        String result = "";
    ...
    public int get_preset_status(int n) {
        setup_status();
    ...
    private String get_preset_str(int n) {
        setup_status();
    ...
    private void setup_timers() {
        ...
        for (int i = 1; i < t10_no_timers + 1; i++) {
            ...
            do; while (!inputs[indx++].matches("Switches.*"));
            ...
            do; while (!inputs[indx++].matches("Weekdays.*"));
            for (int j = 0; j < 7;) {
                if (inputs[indx].matches(".*<input.*")) {
            ...
            do; while (!inputs[indx++].matches("ON Time.*"));
            int on_h = extract_value(inputs[indx++]);
            int on_m = extract_value(inputs[indx++]);

            do; while (!inputs[indx++].matches("OFF Time.*"));
    ...
    public Document xml_config() {
        ...
        Element root = doc.createElement("t10");
    ...
    private void xml() {
        try {
            harcutils.printDOM(null, xml_config(), "t10config.dtd");
    ...
    public static void main(String args[]) {
        ...
        ezcontrol ez = new ezcontrol(ezcontrol_host, verbose);
        ...
        try {
            if (do_get_status) {
                if (num_arg > 0) {
                    ...
                    System.out.println(ez.get_preset_status());
                }
                ...
                ez.send_preset(num_arg, command_name);
            } else {
