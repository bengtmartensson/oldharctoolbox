...
public class home {
    ...
    public home(String home_filename, boolean verbose, int debug, String browser) throws IOException, SAXParseException, SAXException {
    ...
    public home() throws IOException, SAXParseException, SAXException {
        this(harcprops.get_instance().get_homefilename(), false, 0, harcprops.get_instance().get_browser());
    }
    
    public void load(String home_filename) throws IOException, SAXParseException, SAXException {
            ...
            switch (type) {
                ...
                case ir:
                    ...
                    try {
                        if (gw_class.equals("globalcache")) {
                            ...
                            if (db.transmit()) {
                                System.err.println("Trying an Irtrans (" + gw_hostname + ")...");
                            }
                                ...
                                if (preset != -1 && ezcontrol_t10.is_preset_command(cmd))
                                    switch (cmd) {
                                        case get_status:
                                            ...
                                            break;
                                        case set_power:
                                            success = t10.send_preset(preset, power);
                                            break;
                                        default:
                                            success = t10.send_preset(preset, cmd);
                                            break;
                                    }
                                else {
                ...
                case serial:
                    ...
                    if (gw_class.equals("globalcache")) {
                        ...
                        try {
                            if (db.transmit() || verbose)
                                System.err.println("Trying Globalcache (" + gw_hostname + ") for serial using " + gw_connector + ", \"" + subst_transmitstring_printable + "\"");

                            int delay_between_reps = the_command.get_delay_between_reps();
                            int no_read_lines = the_command.get_response_lines();

                            result = gc.send_serial(subst_transmitstring, Integer.parseInt(gw_connector.substring(7)), no_read_lines == 0 ? 0 : 1, count, delay_between_reps);
                            if (no_read_lines > 0)
                                no_read_lines--;
                            Thread.sleep(10);
                            ...
                            for (int i = 0; i < no_read_lines; i++) {
                                String answ = gc.send_serial(null, Integer.parseInt(gw_connector.substring(7)), 1, count, delay_between_reps);
                                System.err.println(">>> " + answ);
                                result = result.isEmpty() ? answ : (result + "\n" + answ);
                            }
                            if (no_read_lines < 0)
                                for (;;) {
                                    result = gc.send_serial(null, Integer.parseInt(gw_connector.substring(7)), 1, count, delay_between_reps);
                                    System.err.println(">>> " + result);
                                }
                            //success = result != null;
    ...
    public String expand_alias(String dname) {
        ...
        if (alias_node != null) {
            ...
            return find_node("device", dname) != null ? dname : null;
        }
    }

    public String get_canonical_name(String dev_name) {
        String can = get_attribute(dev_name, "canonical");
        return can != null ? can : expand_alias(dev_name);
    }
    ...
    // Bug/limitation: Operates on the device classs, not on the instance as defined
    // in the home file. I.e., returns true if a command is available as a serial
    // command, although no serial connection exists and is described in the home file,
    ...
    public String get_attribute(String devname, String attribute) {
        devname = expand_alias(devname);
        Element dev_node = find_node("device", devname);
        if (dev_node == null) {
            System.err.println("Device \"" + devname + "\" not found.");
        ...
        return dev_node.getAttribute(attribute);
    }
    ...
    public String get_deviceclass(String devname) {
        ...
        return dev_node.getAttribute("class");
    }
    ...
    public String get_powered_through(String devname) {
        ...
        if (dev_node == null)
            return null;
        NodeList nl = dev_node.getElementsByTagName("powered-through");
        return (nl.getLength() > 0) ? ((Element)nl.item(0)).getAttribute("device") : null;
    }
